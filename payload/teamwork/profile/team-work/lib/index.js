...
export function apply(ctx) {
  ...
  ctx.inject(['commands'], (commandCtx) => {
    commandCtx.commands.register({
      ...
      handler: ({ agent, rawInput }) => {
        ...
        if (active !== current) agent.session.append('teamwork/state', { active })
  ...
  const migrateLegacy = (agent) => {
    if (agent == null) return
    const folded = foldTeamwork(agent.session.events)
    if (!folded.explicit && lastPreset(agent.session.events) === TEAM_PRESET) {
      agent.session.append('teamwork/state', { active: true })
    }
  }
  ...
  const publishTeamworkState = (agent) => {
    ...
    agent.session.append('teamwork/state', { active })
  }
